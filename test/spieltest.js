...
(function (root) {
  ...
  function laufen(codeLesen) {
    ...
    (function () {
      ...
      let z = spielImGang(0);
      let ev = [];
      for (let i = 0; i < 3; i++) ev = ev.concat(loese(z, uhr, 0));
      const hoch = ev.find(e => e.art === 'stufe');
      pruefe('Stufen', '3 richtige in Folge → Stufe hoch (H → E)', z.stufe === 'E' && hoch && hoch.richtung === 'hoch');
      pruefe('Stufen', 'Meldung „Du bist jetzt in der Expertenstufe!"', hoch && hoch.text === 'Du bist jetzt in der Expertenstufe!');
      for (let i = 0; i < 3; i++) loese(z, uhr, 0);
      ...
      for (let i = 0; i < 3; i++) loese(z, uhr, 0);
      pruefe('Stufen', 'Danach greift die Automatik wieder', z.stufe === 'H');
    ...
    (function () {
      ...
      pruefe('Anstieg', 'Tutorial und Etappe 1–2 auf Niveau 1 (Einstieg)', tutorial === 1 && nur(1, 1) && nur(2, 1));
      pruefe('Anstieg', 'Etappe 3–4 auf Niveau 2 (Aufbau)', nur(3, 2) && nur(4, 2));
      pruefe('Anstieg', 'Ab Etappe 5, im Endlosmodus und bei Großbauwerken: Niveau 3 (voller Zahlenraum)', [5, 6, 7, 8, 9].every(e => nur(e, 3)));
    })();
